...
public class Afficheur {
    public Afficheur(Context context) {
        ...
        //Matrix.setLookAtM(mViewMatrix, 0, 0, 1, 0, 0f, 1f, 1f, 0f, 1.0f, 0.0f);
        //Defines a viewing transformation... TODO try without it
...
//can't use Matrix class if we are in 2x2, can we? -> no, but 3x3 is ok for translations
        ...
        Matrix.setRotateM(mTransfMatrix, 0, mAngle, 0, 0, 1);
...
//frustumM(matrix,offset,left,right,bottom,top,zNear,zFar)
    ...
    public void endInit() {
        ...
        if (!GLES20.glIsEnabled(GLES20.GL_BLEND))
            Log.wtf("OMG ", "LKVNZOEVNLSDOIFHOAEFOEUFOZEF");
